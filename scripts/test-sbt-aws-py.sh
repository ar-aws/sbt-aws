...
generate_tenant_data
tenant_id=$(python3 test.py create-tenant "$TENANT_NAME" "$TENANT_EMAIL" "basic" | jq -r '.data.tenantId')
...
tenants=$(python3 test.py get-all-tenants 30)
...
tenant_details=$(python3 test.py get-tenant "$tenant_id")
...
python3 test.py delete-tenant "$tenant_id" >/dev/null
if [ $? -eq 0 ]; then
...
delete_output=$(python3 test.py delete-tenant "$fake_tenant_id" 2>&1)
...
generate_user_data
user_id=$(python3 test.py create-user "$USER_NAME" "$USER_EMAIL" "basicUser" | jq -r '.data.userName')
...
users=$(python3 test.py get-all-users 30)
...
user_details=$(python3 test.py get-user "$user_id")
...
python3 test.py update-user "$user_id" "$new_user_role" "$new_user_email" >/dev/null
if [ $? -eq 0 ]; then
    log_test "pass" "User update initiated successfully"

    # Get the updated user details
    updated_user_details=$(python3 test.py get-user "$user_id")
...
python3 test.py delete-user "$user_id" >/dev/null
if [ $? -eq 0 ]; then
